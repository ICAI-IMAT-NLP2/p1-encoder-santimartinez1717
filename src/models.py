...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model: int, num_attention_heads: int):
        ...
        self.heads = nn.ModuleList([AttentionHead(d_model, d, d, d) for i in range(num_attention_heads)])
    ...
    def forward(self, hidden_state):
        ...
        outputs = [l(hidden_state) for l in self.heads]
...
class Embeddings(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        position_embeddings = position_embeddings.unsqueeze(0).expand_as(token_embeddings)
